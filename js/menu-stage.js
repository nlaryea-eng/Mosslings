 ...
 *
 *   - world carousel + controls : once Level 1 is cleared
 *   - daily / ghost race        : once the player reaches World 2
 *   - level editor              : once they reach World 3, OR ~week three by tenure
...
const CAROUSEL_WINDOW_RADIUS = 3; // render at most 2*r+1 world cards regardless of count
...
function menuFeatureState({
    ...
    const size = Math.max(1, worldSize | 0);

    const worldMenu = u >= 1;                 // cleared Level 1
    const controls = u >= 1;
    const daily = u >= size;                  // reached World 2
    const editor = u >= size * 2 || days >= editorTenureDays; // World 3 or tenured
    const gallery = editor && customs > 0;

    let stage = 'newcomer';
    if (worldMenu) stage = 'learning';
    if (daily) stage = 'explorer';
    if (editor) stage = 'veteran';

    return { stage, worldMenu, controls, daily, editor, gallery };
}

/**
 * Indices of the worlds to actually render, bounded to a small window around the
 * selection so a 100-world campaign costs the same to render as a 3-world one.
 * For small campaigns it returns every world (so nothing visibly changes today).
 */
